...
var GiniTokenABI = require('../public/javascripts/giniTokenABI');
var GiniLottoABI = require('../public/javascripts/giniLottoABI');
...
// router.post('/BuyLotto', function(req, res) {
//   try {
//     //web3, contract 초기화
//     var web3 = new Web3();
//     //web3.setProvider(new web3.providers.HttpProvider("http://"+etherconfig.ip+":"+etherconfig.port))
//     web3.setProvider(new web3.providers.HttpProvider("https://rinkeby.infura.io/v3/a5d883b8e91348bba8f7e625b35ebdc5"));
//     var contract = new web3.eth.Contract(GiniLottoABI, etherconfig.contractAddr);


//     //admin keystore 접근 및 decrypt
//     var pk;
//     try {
//       var keystore = fs.readFileSync(req.session.passport.user.path, 'utf8');
//       pk = web3.eth.accounts.wallet.decrypt([keystore], req.body.admpwd);
//     }
//     catch (exception) {
...
//     //사용주소설정
//     var myAccount = (pk[0]["address"]);
//     web3.eth.defaultAccount = myAccount;

//     //contract 함수
//     //var _tv = req.body.tokenval;
//     var _tv = web3.utils.toWei(req.body.tokenval.toString(), 'ether')
//     var enabi = contract.methods.transfer(req.body.toaddr, _tv).encodeABI();
//     console.log(enabi);
  
//     //전송
//     web3.eth.sendTransaction({from:myAccount ,to:etherconfig.contractAddr, value:0, gas:53000, gasPrice:10000000, data:enabi})
...
//         req.app.io.in(req.session.passport.user.room).emit('sendtokenmsg', 'confirm');
//       }
//     })
//     .on('error', function(err){
//       console.log("Err > " + err);
//       var _err = err.toString()+"";
//       if(_err.indexOf('reverted') > -1){
//         req.app.io.in(req.session.passport.user.room).emit('sendtokenmsg', 'paused');
//       }
//     }, function(err, result){
//       console.log("Rece > " + rere);
//     });
//   } catch (exception) {
//     console.log("!!!! > "+exception);
